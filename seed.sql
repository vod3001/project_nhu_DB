...
VALUES
    (
        (SELECT id FROM building WHERE name = 'TUAS'),
    ...
    (
        (SELECT id FROM building WHERE name = 'Undergraduate Center'),
    ...
    (
        (SELECT id FROM building WHERE name = 'Computer Science Building'),
...
VALUES
    (
        (SELECT r.id 
        FROM room AS r 
        INNER JOIN building AS b ON r.building_id = b.id
        WHERE b.name = 'TUAS' AND r.name_or_number = 'AS1'),

        (SELECT id FROM equipment_type WHERE name = 'Projector')
    ),
    (
        (SELECT r.id 
        FROM room AS r 
        ...
        (SELECT id FROM equipment_type WHERE name = 'Video camera')
    ),
    (
        (SELECT r.id 
        FROM room AS r 
        ...
        (SELECT id FROM equipment_type WHERE name = 'Microphone')
    ),
    (
        (SELECT r.id 
        FROM room AS r 
        ...
        (SELECT id FROM equipment_type WHERE name = 'Whiteboard')
    );
...
VALUES
    (
        ...
        (SELECT id FROM organization WHERE name = 'AYY'),

        (SELECT id FROM person WHERE name = 'To Lam'),
...
VALUES
    (
        ...
        INNER JOIN person AS p ON bs.created_by = p.id
...
INSERT INTO booking (room_id, created_by, series_id, start_datetime, end_datetime, status, requires_approval)
VALUES 
    (
        (SELECT r.id 
        FROM room AS r 
...
VALUES
    (
        ...
        (SELECT id FROM person WHERE name = 'Luka'),
...
INSERT INTO booking (room_id, created_by, start_datetime, end_datetime, status, requires_approval)
VALUES
    (
        (SELECT r.id 
        FROM room AS r 
